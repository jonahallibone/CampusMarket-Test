angular.module( 'bmuApp.pmessenger', [
  ...
  'angular-jwt',
  'pusher-angular'
]).controller( 'messagescontroller', function MessagesController( $scope, $http, store, $state, $stateParams, UserService, $pusher, $rootScope) {
  ...
  $scope.newPosts = [];
  $scope.inMessage = {};
  $scope.inMessage.isIn = false;

  $scope.$watch('newPosts.new', function (value) {
    $scope.posts = [];
    Array.prototype.push.apply($scope.posts, value);
  });
...
}).controller('messageslistcontroller', function($scope, $http, store, $state, UserService, $pusher, $stateParams, $rootScope) {
  ...
  /*  $http({
    ...
    $http({
      ...
      //console.log(data);
      $scope.inMessage.isIn = true;
      $scope.messages = data.data.messages.reverse();
      $scope.otherUser = data.data.otherUser;
      //UserService promise to get associated user likes

      UserService.getLikes(data.data.otherUser.id).then(function(data) {
        $scope.newPosts.new = data.data.posts;
      });
...
}).controller('newmessagecontroller', function($scope, $http, store, $state, UserService, $pusher, $stateParams) {
