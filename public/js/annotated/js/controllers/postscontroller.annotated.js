angular.module( 'bmuApp.posts', [
  ...
  '720kb.tooltips'
]).controller( 'postscontroller', ["$scope", "$http", "$state", "UserService", "Upload", "$rootScope", "URL_BASE", function PostsController($scope, $http, $state, UserService, Upload, $rootScope, URL_BASE) {
...
angular.module( 'bmuApp.posts').controller( 'categorycontroller', ["$scope", "$http", "store", "$stateParams", "$state", "URL_BASE", "UserService", "$rootScope", function CategoryController( $scope, $http, store, $stateParams, $state, URL_BASE, UserService, $rootScope) {
  ...
  $scope.loadMorePosts = function() {
    ...
    if(!noMorePosts) {
      ...
      $http({
        url: "https://"+URL_BASE+"api/posts?category[]="+$stateParams.id+"&page="+$scope.currentPage,
        method: 'GET'
      }).then(function(data) {
          Array.prototype.push.apply($scope.posts, data.data.posts);
          if(data.data.posts.length < 4) {
...
}]).controller( 'searchcontroller', ["$scope", "$http", "store", "$stateParams", "$state", "URL_BASE", "UserService", "$rootScope", function SearchController( $scope, $http, store, $stateParams, $state, URL_BASE, UserService, $rootScope) {
